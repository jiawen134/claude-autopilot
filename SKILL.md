...
### 3.4 设计探索 & 规划（Superpowers 融合）

**极小需求（一眼看完就能判断）**：直接拆 1-2 个任务，跳到 3.5。

**小/中/大需求**：分两阶段——先设计探索（头脑风暴），再详细规划。

#### 阶段 A：设计探索（头脑风暴）

用 Agent tool 启动 1 个 strategist（model: opus, subagent_type: general-purpose）做设计探索：

1. **读需求**：读 `.claude/state/requirements.md`
2. **读代码**：用 Explore agent 扫描项目结构、核心模块、已有 pattern
3. **提出 2-3 种方案**：
   - 每种方案的架构思路、优缺点、trade-offs
   - 对复杂度、可维护性、性能的影响
   - 实现成本估算（任务数、涉及文件数）
4. **输出设计文档**：写入 `.claude/state/design-spec.md`
5. **⛔ 硬性门控**：用 AskUserQuestion 展示方案摘要，用户批准后才进入阶段 B

> 如果用户说"你看着办"或"快速模式"，选推荐方案并记录理由，跳过等待。
...
3. **自检清单**：
   ...
   - [ ] 函数名、类型、参数在各任务间一致？
   - [ ] 每个任务都有验证命令？
4. **输出规划文档**：写入 `.claude/state/plan.md`，包含架构图、任务列表、依赖关系、风险点
