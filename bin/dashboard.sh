...
if command -v jq &>/dev/null; then
    ...
    for f in "$STATE_DIR"/status-*.json; do
        [ -f "$f" ] || continue
        $FIRST || TEAMMATES_JSON+=","
        FIRST=false
        TEAMMATES_JSON+=$(cat "$f")
    done
    TEAMMATES_JSON+="]"
fi
...
if [ -f "$USAGE_FILE" ] && command -v jq &>/dev/null; then
    ...
    PASS=$(jq -r 'select(.outcome=="pass")' "$USAGE_FILE" 2>/dev/null | wc -l || echo 0)
    FAIL=$(jq -r 'select(.outcome=="fail")' "$USAGE_FILE" 2>/dev/null | wc -l || echo 0)
...
RECENT_EVENTS="[]"
if [ -f "$USAGE_FILE" ] && command -v jq &>/dev/null; then
    RECENT_EVENTS=$(tail -20 "$USAGE_FILE" | jq -s '.' 2>/dev/null || echo "[]")
fi
...
if command -v jq &>/dev/null; then
    echo "$TEAMMATES_JSON" | jq -r '.[] | "<div class=\"teammate\"><div class=\"tm-dot \(.status // "idle")\"></div><div class=\"tm-name\">\(.teammate // "?")</div><div class=\"tm-role\">\(.role // .action // "?")</div><div class=\"tm-action\">\(.detail // .task // .action // "")</div><div class=\"tm-ts\">\(.ts // "")</div></div>"' 2>/dev/null >> "$OUTPUT"
fi
NO_STATUS=$(ls "$STATE_DIR"/status-*.json 2>/dev/null | wc -l)
if [ "$NO_STATUS" -eq 0 ]; then
    echo '<div class="teammate"><div class="tm-dot idle"></div><div class="tm-name">Waiting...</div><div class="tm-action">No teammates active yet. Run /agent-teams to start.</div></div>' >> "$OUTPUT"
fi
echo '</div>' >> "$OUTPUT"
...
if [ -f "$USAGE_FILE" ] && command -v jq &>/dev/null; then
    tail -20 "$USAGE_FILE" | tac | while IFS= read -r line; do
        TS=$(echo "$line" | jq -r '.ts // ""' 2>/dev/null)
        HOOK=$(echo "$line" | jq -r '.hook // ""' 2>/dev/null)
        TM=$(echo "$line" | jq -r '.teammate // ""' 2>/dev/null)
        ACTION=$(echo "$line" | jq -r '.action // .outcome // ""' 2>/dev/null)
        DUR=$(echo "$line" | jq -r '.duration_s // ""' 2>/dev/null)
        CLS="skip"
        [ "$ACTION" = "pass" ] && CLS="pass"
        [ "$ACTION" = "fail" ] || [ "$ACTION" = "test_fail" ] || [ "$ACTION" = "lint_fail" ] && CLS="fail"
        echo "<div class=\"evt\"><span class=\"$CLS\">${TS}</span>  ${TM}  ${HOOK}  ${ACTION}  ${DUR}s</div>" >> "$OUTPUT"
    done
else
...
# Footer
cat >> "$OUTPUT" << 'HTMLFOOT'
<div class="footer">Auto-refreshes every 5 seconds. Generated by /agent-teams skill.</div>
</body>
</html>
HTMLFOOT
